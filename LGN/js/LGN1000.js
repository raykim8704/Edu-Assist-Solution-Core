...
var page = {
	            ...
	            if (!block) {
	                ...
	                if (!validationResult) {
	                    ...
	                    if (loginResult.customStatus === "success") {
	                                    ...
	                                    LEMP.EDUApp.getLomeoUuid({
	                                        "_fCallback": function(res) {
	                                            if (res.result === "true" || res.result === true) {
	                                                ...
	                                                LEMP.Window.open({"_sPagePath": "MAN/html/MAN1000.html"});
	                                            } 
	                                            else {
	                                                LEMP.Window.open({"_sPagePath": "MAN/html/MAN1000.html"});
	                                            }
	                        ...
	                        LEMP.Properties.set({_sKey: "autoLoginState", _vValue: false});
	...
	setLoginInfo : function(loginResult, pw, autoLoginState) {
	    ...
	    LEMP.Properties.get({ "_sKey" : "loginBefore"}) ?  LEMP.Properties.set({
	        "_sKey" : "loginBefore",
	        "_vValue" : true
	    }) :  
	    	LEMP.Properties.set({
...
var callbackBackButton = function() {
    ...
    var cancleBtn = LEMP.Window.createElement({_sElementName: 'TextButton'});
    cancleBtn.setProperty({"_sText": "아니요"});

    LEMP.Window.confirm({
        "_sTitle": "INSIGHT 종료",
        "_vMessage": "INSIGHT를 종료하시겠습니까?",
        "_aTextButton": [agreeBtn, cancleBtn]
    });
}
