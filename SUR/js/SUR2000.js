...
function renderMultiple(obj,index){
	...
	for ( var i = 0; i< obj.example.length ;i++){
		$('#form-'+index).append($('<p/>',{ id : 'p-'+index+'-'+i }));
		$('#p-'+index+'-'+i).append($('<input/>',{ class : 'with-gap', name : 'group-'+index, 'type' : type, id : 'input-'+index+'-'+i, 'value' : i+1 }));
		$('#p-'+index+'-'+i).append($('<label/>',{ 'for' : 'input-'+index+'-'+i, text : obj.example[i].content }));
	}
}
