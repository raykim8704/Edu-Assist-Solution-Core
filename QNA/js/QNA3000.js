	...
	initInterface : function(questionInfo) {
		...
		$('#btn-write-comment').click(function(){
			...
			if(commentContent){
				...
				var registComment = INSIGHT.REST.registComment(eventID,classNum,qnum, token,comment);
			...
			start = swipe[0].pageY;
	    	    ...
	    	    $(this).on('touchmove', function(e) {
	    	        ...
	    	        if (distance < -30){
	    	            ...
	    	            $('#textarea-reply').blur();
	    	            $('#textarea-reply').css({height:'1rem'});
...
//(GET - /event/{이벤트번호}/class/{클래스번호}/qna/{qna번호}/comment
	var replyList = INSIGHT.REST.getQnaCommnets(token, eventID,classNum,qnum);
...
//             text: "삭제된 데이터는 ",
             ...
             function(isConfirm){
             if (isConfirm) {
            	 var res = INSIGHT.REST.deleteComment(token,eventID,classNum,qnum,cnum);
            	 if(res.code == 200){
            		 renderReplys(qnum);
            	 }
             }
