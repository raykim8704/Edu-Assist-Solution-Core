...
var page = {
	...
	init : function(eventInfo) {
		...
		console.log(eventID);
         ...
         authority = LEMP.Properties.get({"_sKey": "authority"});
...
function reRender(args){
	console.log('sdfsdfdsfsdfdsfdsfsdfsdfsdfdsfsdfsdfsdf', args);
...
var renderQuestionList = function(eventID, refresh = false){
	...
	$('.material-icons').css({'margin-right':'8px'});
	$('.card-header').css({'margin-bottom':'0px'});
	$('.card-content').css({padding: '10px'});
	$('.like-count').css({'padding-top' : '0px'});
	$('.like-count').css({'font-size':'11px','color':'#bdbdbd'});
	$('.qna-date').css({'font-size':'11px','color':'#bdbdbd'});
	setClickEvents();
	}
	}else{
...
function setClickEvents() {
	...
	$('.dropdown-del').click(function(){
		...
		swal({    title: "해당 글을 삭제하시겠습니까?",
            ...
            function(){
            	var deleteQna = INSIGHT.REST.deleteQna(eventID,classNum, qnum, token);
