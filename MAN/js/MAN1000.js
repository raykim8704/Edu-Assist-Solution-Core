...
LEMP.addEvent("backbutton", "callbackBackButton"); //안드로이드 back 버튼 입력 이벤트
var token;
...
var page = {
		...
		initLayout : function(){
	         token = LEMP.Properties.get({"_sKey": "token"});
		...
		initInterface : function(){
			$('#btn-logout-man').click(function(){
				 ...
				 swal({   title: "Logout",
		              ...
		              if (isConfirm) {
		            	  var result = true;
					        LEMP.EDUApp.stopLomeoPush({"_fCallback": function(res) {}});
					        ...
					        LEMP.EDUApp.getLomeoUuid({
					            "_fCallback": function(res) {
					                ...
					                LEMP.Properties.remove({"_sKey": "autoLoginState"});
					                LEMP.Properties.remove({"_sKey": "userID"});
					                LEMP.Properties.remove({"_sKey": "userPw"});
					                


					                LEMP.Window.open({"_sPagePath": "LGN/html/LGN1000.html"});
					            }
					        });
		...
		renderEventList : function(eventList){
	        ...
	        if (!isEmptyCard){ 
	        	...
	        	LEMP.Properties.set({'_sKey': 'evnum', '_vValue': ev})
	        	LEMP.Properties.set({'_sKey':'evtitle','_vValue':title});
	            var authLevel = LEMP.Properties.get({"_sKey": "authority"});
...
function renderList(v,i){
	var target = (v.end) ? '#ended' : '#ongoing' ; 
     ...
     $('#' + v.id).append($('<div/>', {class: 'card-image'}));
     if (v.image != null) {
     ...
     $('#' + v.id).append($('<div/>', {class: 'card-content'}));

     //카드 안에 설명 추가
     $('#' + v.id).children('.card-content').append($('<p/>', {class: 'content-title'})).append($('<p/>', {text: v.description})).append($('<p/>', {class: 'content-term'}));

     //카드 안에 제목 추가
     $('#' + v.id).children('.card-content').children('.content-title').append($('<span/>', {text: v.title,id:'content-title-'+v.id})).css({"color": "#2c2a29", "font-size": "14px"});
...
var callbackBackButton = function() {
    var agreeBtn = LEMP.Window.createElement({_sElementName: 'TextButton'});
    agreeBtn.setProperty({
        "_sText": "종료하기",
        "_fCallback": function() {
            LEMP.App.exit();
        }
